...
- Crate `talaria-sources`: `SourceConnector`, `SourceRegistry`, `PlanSources`, fixture/Wikidata/Wikipedia/Wikisource/Commons plus **live catalog** connectors (HAL, Persée, Gallica, theses.fr, OpenAlex, Open Library, Internet Archive, BnF; Europeana when `EUROPEANA_API_KEY` is set). Remaining Lot C/D sources (VIAF/ISNI/IdRef) are still stubs.
...
- **Stubs ≠ integrations**: announce maturity via `source-status` only. Wikipedia, Wikisource, Commons, Wikidata, HAL, Persée, Gallica, theses.fr, OpenAlex, Open Library, Internet Archive, and BnF are fetch/parse/extract ready with `--live`. Europeana is ready only with `EUROPEANA_API_KEY`. VIAF/ISNI remain stubs.
