...
- **Stubs ≠ integrations**: announce maturity via `source-status` only. HAL, Persée, Gallica, theses.fr, OpenAlex, Open Library, Internet Archive, and BnF are fetch/parse/extract ready with `--live`. Europeana is ready only with `EUROPEANA_API_KEY`. Wikisource/Commons are extraction_ready with `--live`. VIAF/ISNI remain stubs.
