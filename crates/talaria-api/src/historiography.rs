...
use talaria_store::{
    ...
    run_migrations, search_local_entities, ClaimInsert,
};
use talaria_text::split_sentences;
use uuid::Uuid;

pub async fn run_historiography_extract(config: &AppConfig, subject: &str) -> anyhow::Result<()> {
    let pool = connect(config).await?;
    run_migrations(&pool).await?;
    let (entity_id, label, wiki_title) = resolve_subject(&pool, &config.wiki_lang, subject).await?;
